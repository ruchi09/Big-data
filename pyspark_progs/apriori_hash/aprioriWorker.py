...
def nextFrequent_hash(itemsets,minSupport):
    # global minSupport
    bucket_size = 13
    ...
    for i in itemsets:
		...
		for j in range(0, len(i)-1):
			for k in range(j+1,len(i)):
				hash = (l[j]*10 + l[k])%bucket_size
				hashtable[hash].append((l[j],l[k]))
...
def get_frequent_items_sets(data,min_sup,steps=0):
	...
	while L_set != set([]) and k != steps+1:
		...
		if k<3:
			C_set=nextFrequent_hash(transactions,min_sup)
		else:
	...
	print "\nLocally frequent items=", solution
	return solution
